...
def load_model(model_path, model_type, device, **kwargs):
    ...
    if model_type == 'ddpm':
        model = DDPM(num_timesteps=kwargs.get('num_steps', 1000))
    elif model_type == 'ddpm_cond':
        model = ConditionalDDPM(num_classes=kwargs.get('num_classes', 10), num_timesteps=kwargs.get('num_steps', 1000))
...
def main():
    ...
    if os.path.exists(args.ddpm_path):
        ...
        model = load_model(args.ddpm_path, 'ddpm', device, num_steps=args.num_steps)
        samples = sample_ddpm(model, device, args.num_samples, args.sampling_steps, mask)
        filename = f'samples_ddpm.pt'
    ...
    if os.path.exists(args.ddpm_cond_path):
        ...
        model = load_model(args.ddpm_cond_path, 'ddpm_cond', device, num_steps=args.num_steps, num_classes=10)
