...
def sample(model, device, num_samples=16, num_steps=1000):
    ...
    for t in tqdm(range(num_steps-1, -1, -1), desc="Sampling"):
        ...
        predicted_noise = model(samples, t_batch)
...
if __name__ == "__main__":
    ...
    model = DDPM(num_classes=10, num_timesteps=args.num_steps)
    model.to(device)
    
    run_name = f"exps_ddpm/{args.epochs}ep_{args.batch_size}bs_{args.learning_rate}lr_{args.num_steps}numsteps_{args.mask}mask" 
