...
def train(model, train_loader, test_loader, run_name, learning_rate, epochs, batch_size, num_steps, mask_type, device):
    ...
    scheduler = MaskSchedulerD3PM(num_timesteps=num_steps, mask_type=mask_type, mask_start=0.0, mask_end=0.95)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            predicted_logits = model(masked_images, timesteps, labels)
...
def sample(model, class_label, device, num_samples=16, num_steps=1000):
    ...
    scheduler = MaskSchedulerD3PM(num_timesteps=num_steps, mask_type="linear", mask_start=0.0, mask_end=0.95)
    scheduler.mask_probs = scheduler.mask_probs.to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="D3PM Conditional Model")
    ...
    parser.add_argument("--mask_type", type=str, default="linear", help="Type of mask scheduling")
...
if __name__ == "__main__":
    ...
    run_name = f"exps_conditional_d3pm/{args.epochs}ep_{args.batch_size}bs_{args.learning_rate}lr_{args.num_steps}steps_{args.mask_type}mask"
    os.makedirs(run_name, exist_ok=True)

    if args.mode == "train":
        model.train()
        train(model, train_loader, test_loader, run_name, args.learning_rate, args.epochs, args.batch_size, args.num_steps, args.mask_type, device)
